...
import { Experience as ExperienceModel, Position } from '../../models/cv-data.interface';
import {
...
export class Experience {
  ...
  private readonly positionStates = signal<Map<string, Map<number, boolean>>>(
    new Map(),
  );
  ...
  protected isCompanyExpanded(expId: string): boolean {
    return (
      this.companyStates().get(expId) ?? this.CONFIG.defaults.companyExpanded
    );
  }
  ...
  protected isPositionExpanded(expId: string, posIndex: number): boolean {
    return (
      this.positionStates().get(expId)?.get(posIndex) ??
      this.CONFIG.defaults.positionExpanded
    );
  }
  ...
  protected getTotalTechnologies(): number {
    ...
    this.experiences().forEach((exp) => {
      this.getPositions(exp).forEach((pos) =>
        pos.technologies?.forEach((t) => all.add(t)),
      );
