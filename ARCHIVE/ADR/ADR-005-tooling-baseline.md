...
**Coverage expectation is deliberately modest:** the goal is a _working gate plus
characterization tests for the code that survives the deletions_, not a coverage target.
...
- **Sequencing nuance:** Sheriff's value is realized only once the `domains/` structure
  exists (ADR-001 move). So Sheriff is _installed_ in this ADR but its rules are
...
**B. Skip tests, rely on build + manual smoke.** Rejected as the _permanent_ answer
(it's the acceptable _temporary_ one). A deletion program touching `CvDataService` with
...
**DECIDEs (resolved 2026-06-14):**

- **D1:** vitest integration path — **`@analogjs/vitest-angular`** (already in use;
  vitest 76/76 passing). No change needed.
- **D2:** coverage threshold — **none** (gate only to start; add a floor once
  characterization tests exist).
- **D3:** lint strictness ceiling — **lenient** initially (0 errors, ~80 warnings);
  tighten incrementally after ADR-002 deletes the heaviest `any` offenders.
...
DECIDEs recorded; status _Accepted_.
